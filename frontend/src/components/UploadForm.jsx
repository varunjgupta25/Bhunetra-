...
import { getDocumentTypeLabel } from '@/components/DocumentTypeSelector'
...
export function UploadForm({ onComplete, hidePipeline = false, documentType = null, onChangeDocumentType = null }) {
  ...
  const handleStartPipeline = async () => {
    ...
    try {
      ...
      // Pass the selected document type (from the new selection step) if available
      if (documentType) {
      ...
      <div className={hidePipeline ? "w-full space-y-gutter" : "lg:col-span-7 space-y-gutter"}>
        <div className="bg-surface-container-lowest rounded-[20px] p-card-padding card-shadow border border-[#D0E8F5]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            {/* Document Category — shown as read-only label when documentType prop provided, editable select otherwise */}
            <div>
              <label className="block font-label-sm text-label-sm text-on-surface mb-2">
                Document Category
              </label>
              {documentType ? (
                <div className="w-full bg-surface-container-lowest border border-[#B8D8EE] rounded-lg px-4 py-3 font-body-md text-body-md text-on-surface flex items-center gap-2 opacity-80">
